      ...
      <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {images.map((image, index) => (
          <Reveal key={index} delay={(index % 4) * 0.08}>
            ...
            >
              <Image src={image} alt={`Raj Garden gallery image ${index + 1}`} fill sizes="(min-width: 1024px) 25vw, (min-width: 768px) 33vw, 50vw" className="object-cover transition duration-700 group-hover:scale-110" />
      ...
      {active ? (
        <div className="fixed inset-0 z-[110] grid place-items-center bg-charcoal/90 p-5" onClick={() => setActive(null)}>
          ...
          <div className="relative h-[82vh] w-full max-w-5xl overflow-hidden rounded-[28px]" onClick={(event) => event.stopPropagation()}>
            <Image src={active} alt="Selected Raj Garden gallery image" fill sizes="90vw" className="object-contain" />
          </div>
        </div>
      ) : null}
    </>
